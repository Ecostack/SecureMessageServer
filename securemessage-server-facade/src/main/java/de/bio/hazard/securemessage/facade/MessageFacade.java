...
public class MessageFacade {
	...
	public void addMessage(MessageWebserviceDTO pMessageWebserviceDTO)
		...
		if (lcMessageWebserviceDTO == null) {
			throw new IllegalArgumentException(
					"MessageWebserviceDTO is not allowed to be null");
		}
		Device lcDevice = authenticationService
				.getDeviveWhenAuthTokenIsValidWithException(lcMessageWebserviceDTO
						.getTokenId());
		User lcSender = lcDevice.getUser();
		List<Message> lcMessages = extractMessagesFromMessageWebserviceDTO(
				lcMessageWebserviceDTO, lcSender);
	...
	public RequestMessageWebserviceReturnDTO getMessages(
		...
		Device lcDevice = authenticationService
				.getDeviveWhenAuthTokenIsValidWithException(lcRequestMessageDTO
						.getTokenId());
		User lcSender = lcDevice.getUser();
		List<Message> lcUserMessages = messageService
				.getMessagesByReceiver(lcSender);
		// TODO andere Receiver �bermitteln
		for (Message lcMessage : lcUserMessages) {
			MessageWebserviceDTO lcMessageWebserviceDTO = new MessageWebserviceDTO();
			List<MessageContent> lcMessageContents = messageContentService
			...
			for (MessageContent lcMessageContent : lcMessageContents) {
				...
				lcContentWebserviceDTO.setData(new String(lcMessageContent
						.getData()));
				lcContentWebserviceDTO.setFilename(lcMessageContent
						.getFilename());

				MessageContentKey lcMessageContentKey = messageContentKeyService
						.getMessagesContentKeysByMessageAndMessageContent(
								lcMessage.getId(), lcMessageContent.getId());
				MessageContentKeyWebserviceDTO lcContentKeyWebserviceDTO = new MessageContentKeyWebserviceDTO();
				lcContentKeyWebserviceDTO
						.setSymmetricEncryptionKey(lcMessageContentKey
								.getSymmetricEncryptionKey());
				lcContentWebserviceDTO.getSymmetricKeys().add(
						lcContentKeyWebserviceDTO);
		...
		lcRequestMessageReturnDTO = encryptRequestMessageWebserviceReturnDTO(
				lcRequestMessageReturnDTO, lcDevice);
	...
	private List<Message> extractMessagesFromMessageWebserviceDTO(
			MessageWebserviceDTO pMessageWebserviceDTO, User pSender) {
		List<Message> lcResult = new ArrayList<Message>();
		for (MessageReceiverWebserviceDTO lcMessageReceiverWDTO : pMessageWebserviceDTO
				.getReceiver()) {
			Message lcMessage = new Message();
			User lcReceiver = userService
					.getUserByUsername(lcMessageReceiverWDTO.getUsername());
	...
	private void extractAndAddMessageContent(
			MessageWebserviceDTO lcMessageWebserviceDTO,
			List<Message> lcMessages) {
		for (MessageContentWebserviceDTO lcMessageContentWDTO : lcMessageWebserviceDTO
				.getContent()) {
			...
			for (MessageContentKeyWebserviceDTO lcMessageContentKeyWDTO : lcMessageContentWDTO
					.getSymmetricKeys()) {
				for (Message lcMessage : lcMessages) {
					// XXX oder Vergleich �ber User?
					if (lcMessage.getReceiver().getUsername()
							.equals(lcMessageContentKeyWDTO.getUsername())) {
						MessageContentKey lcMessageContentKey = new MessageContentKey();
						lcMessageContentKey
								.setSymmetricEncryptionKey(lcMessageContentKeyWDTO
										.getSymmetricEncryptionKey());
						lcMessageContentKey.setMessageContent(lcMessageContent);
						lcMessageContentKey.setMessage(lcMessage);
						messageContentKeyService
								.addMessageContentKey(lcMessageContentKey);
					}
				}
			}
		}
	...
	private MessageWebserviceDTO decryptMessageWebserviceDTO(
			MessageWebserviceDTO pMessageWebserviceDTO)
			throws EncryptionExceptionBiohazard {
		...
		try {
			...
			byte[] lcSymmetricKey = encryptionObjectModifier
					.asymmetricDecryptToByte(
							lcMessageWebserviceDTO.getSymEncryptionKey(),
							lcServerPrivateKey.getValue(), true);

			lcMessageWebserviceDTO.setTokenId(encryptionObjectModifier
					.symmetricDecrypt(lcMessageWebserviceDTO.getTokenId(),
							lcSymmetricKey));
			for (MessageContentWebserviceDTO mcwDTO : lcMessageWebserviceDTO
					.getContent()) {
				mcwDTO.setData(encryptionObjectModifier.symmetricDecrypt(
						mcwDTO.getData(), lcSymmetricKey));
				mcwDTO.setFilename(encryptionObjectModifier.symmetricDecrypt(
						mcwDTO.getFilename(), lcSymmetricKey));
				for (MessageContentKeyWebserviceDTO mckwDTO : mcwDTO
						.getSymmetricKeys()) {
					mckwDTO.setUsername(encryptionObjectModifier
							.symmetricDecrypt(mckwDTO.getUsername(),
									lcSymmetricKey));
					mckwDTO.setSymmetricEncryptionKey(encryptionObjectModifier
							.symmetricDecrypt(
									mckwDTO.getSymmetricEncryptionKey(),
									lcSymmetricKey));
				}
			}
		} catch (Exception e) {
	...
	private RequestMessageWebserviceDTO decryptRequestMessageWebserviceDTO(
			RequestMessageWebserviceDTO pRequestMessageDTO)
			throws EncryptionExceptionBiohazard {
		...
		try {
			...
			byte[] lcSymmetricKey = encryptionObjectModifier
					.asymmetricDecryptToByte(
							lcRequestMessageWebserviceDTO.getSymEncryptionKey(),
							lcServerPrivateKey.getValue(), true);
			lcRequestMessageWebserviceDTO.setTokenId(encryptionObjectModifier
					.symmetricDecrypt(
							lcRequestMessageWebserviceDTO.getTokenId(),
							lcSymmetricKey));
		} catch (Exception e) {
			// TODO SebastianS; Logging
			e.printStackTrace();
	...
	private RequestMessageWebserviceReturnDTO encryptRequestMessageWebserviceReturnDTO(
			RequestMessageWebserviceReturnDTO pRequestMessageReturnDTO,
		...
		try {
			...
			// TODO NicoH; DEBUG!!
			new byte[] { 57, -104, 10, -47, 66, 8, -17, -19, 126, -23, 92, -92,
					-116, -66, 111, 100 };

			lcRequestMessageWebserviceReturnDTO
					.setSymEncryptionKey(encryptionObjectModifier
							.asymmetricEncrypt(lcSymmetricKey,
									lcDevicePublicKey, false));
			System.err
					.println("encryptedKey: "
							+ lcRequestMessageWebserviceReturnDTO
									.getSymEncryptionKey());
			for (MessageWebserviceDTO lcMessage : lcRequestMessageWebserviceReturnDTO
					.getMessages()) {
				for (MessageContentWebserviceDTO lcMessageContent : lcMessage
						.getContent()) {
					lcMessageContent.setData(encryptionObjectModifier
							.symmetricEncrypt(lcMessageContent.getData(),
									lcSymmetricKey));
					lcMessageContent.setFilename(encryptionObjectModifier
							.symmetricEncrypt(lcMessageContent.getFilename(),
									lcSymmetricKey));
					for (MessageContentKeyWebserviceDTO lcMessageContentKey : lcMessageContent
							.getSymmetricKeys()) {
						lcMessageContentKey
								.setSymmetricEncryptionKey(encryptionObjectModifier
										.symmetricEncrypt(lcMessageContentKey
												.getSymmetricEncryptionKey(),
												lcSymmetricKey));
					}
				}
